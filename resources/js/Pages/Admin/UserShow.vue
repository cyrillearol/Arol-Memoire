    ...
    <AuthenticatedLayout>
        <div class="grid gap-8 xl:grid-cols-[360px_1fr]">
            <aside class="tl-card h-fit p-6">
                ...
                <p class="mt-1 text-slate-600">{{ userItem.email }}</p>
                ...
                <div class="mt-6 grid gap-2">
                    <Link v-if="userItem.status !== 'suspendu'" :href="route('admin.users.suspend', userItem.id)" method="patch" as="button" class="rounded-lg border border-red-200 px-4 py-3 text-sm font-bold text-red-600">Suspendre</Link>
                    <Link v-else :href="route('admin.users.activate', userItem.id)" method="patch" as="button" class="tl-button-primary">Réactiver</Link>
                </div>
            </aside>
